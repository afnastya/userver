...
bool easy::check_retry_budget() const {
  return multi_->CheckRetryBudget(effective_url_host_);
}

void easy::update_retry_budget(const std::error_code& err,
                               bool successful_response_code) {
  multi_->UpdateRetryBudget(effective_url_host_, err, successful_response_code);
}
...
native::curl_socket_t easy::opensocket(
  ...
  if (multi_handle) {
    ...
    self->set_retry_budget_factors(url.data());
...
void easy::set_retry_budget_factors(const char* url_str) {
  std::error_code ec;
  curl::url url;
